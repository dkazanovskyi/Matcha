  ...
  import { findIndex, propEq, uniqBy, prop } from 'ramda'
  ...
  class Profile extends React.Component {
    ...
    handleChangeExtraPhotos = (info) => {
      ...
      fileListExtra = uniqBy(prop('name'), fileListExtra)
      ...
      this.props.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.props.updateProfile({
            ...
            tags: values.tags,
            sexualPreferences: values.sexualPreferences
    ...
    beforeUploadEvent = (file, fileList) => {
      if (findIndex(propEq('name', file.name), this.state.fileListExtra) !== -1 && this.state.fileListExtra.length > 0) {
        return false
      }
      return true
    }
              ...
              >
                <div className="dropbox">
                  {getFieldDecorator('dragger', {
                    ...
                    <Upload.Dragger
                      beforeUpload={this.beforeUploadEvent}
                      onChange={this.handleChangeExtraPhotos}
                      fileList={this.state.fileListExtra}
                      disabled={this.state.fileListExtra > 3}
                      name="photos"
                      action="/image-upload/photos"
                      accept="image/*">
  ...
  const mapDispatchToProps = dispatch => {
    return {
      updateProfile: (payload, as, af) => dispatch(UserActions.updateUserProfileRequest(payload, as, af))
    }
  }
export default connect(null, mapDispatchToProps)(WrappedProfile)